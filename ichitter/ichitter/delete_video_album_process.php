...
if($user_id != ""){
		...
		curl_setopt($ch, CURLOPT_URL, SERVICE_NAME . 'delete_video_album_service.php' );
		...
		$curl_data = array(
			...
			"delete_album"=>"delete");
		$curl_data+=$REQ_SEND;
		$curl_call = new INIT_PROCESS(DELETE_VIDEO_ALBUM_SERVICE, $curl_data);
		...
		if($successmsg == "Album deleted successfully"){
		 header('Location: videos.php?msg=Adelsuccess');
		}else{
		 header('Location: videos.php?msg=Adelerror');
		}
}	
?>