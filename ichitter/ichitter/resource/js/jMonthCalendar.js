	...
	var defaults = {
			...
			onMonthChanged: function(dateIn) {
                    $('div.Event').each(function(){                          
                           $(this).parent().append('<div class="indicate" style="width:1px;height:1px;border:1px solid red"></div>');
                      });	
                   return true;
               },
			...
			onDayLinkClick: function(date,whr) {return true;},
...
//			});
		...
		do {
			...
			for (var i=0; i<7; i++) {
				...
				//DateBox Events
				var dateLink = jQuery('<a class="modal"><div class="DateLabel">' + _currentDate.getDate() + '</div></a>').click(function(e) {
                    defaults.onDayLinkClick(new Date($(this).parent().attr("date")),this);
                    e.stopPropagation();
                });
			...
			jQuery.each(calendarEvents, function(){
				...
				if(sDt) {                        
					if ((sDt >= _beginDate) && (sDt <= _endDate)) {
						...
						var link = jQuery('<a class="'+ev.event_user+'" rel="'+ev.Description+'" href="' + ev.URL + '">' + ev.Title + '</a>');
                              
						link.click(function(e) {
							defaults.onEventLinkClick(ev);
							e.stopPropagation();
						});
						var event = jQuery('<div rel="'+ev.show_rel+'" class="Event" id="Event_' + ev.EventID + '"></div>').append(link);
