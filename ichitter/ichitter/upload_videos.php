...
<script language="JavaScript" type="text/javascript">
 ...
 function multiple_filecheck(){
	...
	if (gallery==null || gallery==""){
	   alert("No gallery selected! Please create or select a gallery to upload Videos");
	   return false;
	}	
	if(cnt == 1){
	   alert("please select Video the upload");
	   return false;
	}else{
		...
		var extn = nodes.substring(nodes.lastIndexOf('.') + 1);
         if(nodes != ""){
			if(extn == "mp4" || extn == "mpg" || extn == "mov" || extn == "avi" || extn == "flv"){
			 document.frmUploadmultiple.submit();
			}else{
			 alert("Upload MP4 or MPG or MOV or AVI or FLV Videos only!");
			 return false;
			}		 
		 }
		}
...
<div class="line"></div>
		  <div id="leftmenulist">
			<?php include ('common/side_navigation.php');?>		  
				...
				<form  enctype="multipart/form-data" action="video_upload_process.php" name="frmUploadmultiple" method="POST">							
					<div class="ucipublishedtxt" id="regd-form">
					   <ul class="">
							 <div class="reg-bg" style="padding:0;">
								...
								curl_setopt($ch, CURLOPT_URL, SERVICE_NAME . 'get_video_gallery_service.php' );
								...
								$curl_data = array(
									...
									"upload"=>"Upload" );
								$curl_data+=$REQ_SEND;
								$curl_call = new INIT_PROCESS(GET_VIDEO_GALLERY_SERVICE, $curl_data);
								...
								?>
								<OPTION VALUE="0">Individual Videos</OPTION>
								</SELECT>	
					...
					<div class="ucipublishedtxt">
					   <ul class="flt">
							 <div id="searchuploadGallery">
									...
									<input type="file" style="margin:5px;float:left;" name="multiplefile[]" id="multiplefile[]" class="file" >
									<!-- <input type="file" name="multiplefile[]" id="multiplefile[]" class="file" >
									<input type="file" name="multiplefile[]" id="multiplefile[]" class="file" >
									<input type="file" name="multiplefile[]" id="multiplefile[]" class="file" >
									<input type="file" name="multiplefile[]" id="multiplefile[]" class="file" > -->
									<!-- Button to invoke the click of the File Input -->
									<a href="javascript:;"><img class="searchBtn" src="resource/images/upload-bt.png" onclick="return multiple_filecheck();" /></a>
							</div>							
					   </ul>				
					</div> 
				</form>					
