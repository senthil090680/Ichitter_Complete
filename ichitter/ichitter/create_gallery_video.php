...
//error_reporting(0);
	...
	curl_setopt($ch, CURLOPT_URL, SERVICE_NAME .'create_video_gallery_service.php' );
	...
	$curl_data = array(
		...
		"upload"=>"Upload");
	$curl_data+=$REQ_SEND;
	$curl_call = new INIT_PROCESS(CREATE_VIDEO_GALLERY_SERVICE, $curl_data);
	...
	if($successmsg == "Directory has been created successfully"){
	 $path = $target_path;
	 header('Location: videos.php?msg=glrysuccess');
    //unlink($path);
	}elseif($errormsg == "Directory created already"){
	 header('Location: videos.php?msg=direxist');
	}else{
	 header('Location: videos.php?msg=cnntcrtdir');
	}	
