...
class upload_gallery{
    ...
    function _upload_image($file,$file_name,$user_id,$gallery,$galleryid){
         ...
         $thisdir = getcwd();
		if(!is_dir($thisdir ."/upload/photos/".$user_id)){ 
			mkdir($thisdir ."/upload/photos/".$user_id , 0777, true);
		...
		if($execute){	
			if(isset($_POST['whr'])){
				$msg = array('id' => mysql_insert_id(),'img_name' => $file_name);
			}else{
...
if(isset($post_upload)){
  ...
  $msgs = $upload_obj->_upload_image($file,$file_name,$user_id,$gallery_name,$gallery);
  if(!empty($msgs)){
  $arr_success['success'] = $msgs;
